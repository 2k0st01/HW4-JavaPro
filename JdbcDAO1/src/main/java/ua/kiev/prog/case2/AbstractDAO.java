...
public abstract class AbstractDAO<T> {
    ...
    public void add(T t) {
        try {
            ...
            try (PreparedStatement statement = conn.prepareStatement("SELECT id FROM " + table)) {
    ...
    public List<T> getAll(Class<T> cls, String withFiled) {
        List<T> res = new ArrayList<>();

        try {
            try (Statement st = conn.createStatement()) {
                try (ResultSet rs = st.executeQuery("SELECT "+ withFiled + " FROM " + table )) {
                    ResultSetMetaData md = rs.getMetaData();
